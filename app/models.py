...
class SearchableMixin:
    ...
    def search(cls, expression, page, per_page):
        # Use the query_index function (assuming it's properly secured)
        ids, total = query_index(cls.__tablename__, expression, page, per_page)
        if total == 0:
            return [], 0
        
        # Prepare a case statement for ordering the results based on the order of the IDs
        when = [(id, index) for index, id in enumerate(ids)]
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    map_data: so.Mapped[Optional[str]] = so.mapped_column(sa.Text, nullable=True)
...
class Entry(db.Model):
    ...
    geolocation = db.Column(db.String(100), nullable=False)
    text = db.Column(db.String(200), nullable=False)
    emoji = db.Column(db.String(10), nullable=False)

    def __repr__(self):
        return f'<Entry {self.id}, {self.geolocation}, {self.text}, {self.emoji}>'
